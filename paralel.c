...
int main(int argc, const char* argv[]){
	...
	while(k < nr_saptamani+1)
	{
		...
		#pragma omp parallel for schedule(runtime) shared(nr_culori, matrix) private(i, j, i2, j2, i3, dist_min, ind, max)
		for(i=0; i<N; i++)
		{
			for(j=0; j<N; j++)
			{		
				...
				// nu e necesara pragma omp critical pentru fiecare i,j pentru ca e unica perechea.
